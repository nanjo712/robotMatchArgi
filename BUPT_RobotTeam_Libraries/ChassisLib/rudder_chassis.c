...
void RudderChassis_Move(float speed, float dir, float omega)
{
    ...
    if (fabs(speed) > 1e-4 || fabs(omega) > 1e-4 )
    {
        for (int i = 0; i < 4; i++)
        {
    ...
    else // 速度和角速度都为0时要保证舵向有效
    {
        for (int i = 0; i < 4; i++)
        {
            ...
            RM_MotorStatus[i].pos = dir * STEER_WHEEL_REDUCTION_RATIO;
...
void RudderChassis_SingleCtrl(int index, float speed, float dir)
{
    ...
    //向驱动板发送命令
    dji_PosCtrl(1,index,__RAD2ANGLE(dir * STEER_WHEEL_REDUCTION_RATIO));
