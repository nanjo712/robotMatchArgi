...
void SW_MotorInit(struct SW_DriveMotor_t *driver_motor, struct SW_SteerMotor_t *steer_motor)
{
    for (int i = 0; i < 4; i++)
    {
        ...
        driver_motor->can_id[i] = i + DRIVE_WHEEL_VESC_ID_BASE; // 设置本杰明电调ID
        steer_motor->now_pos[i] = 0;

//        MotorOn(CAN1, i);
        dji_MotorOn(1, i+1);
        dji_PosCfg(1,i+1,STEER_WHEEL_MAX_SPEED);
...
//        return;
    for (int i = 0; i < 4; i++)
    {
        ...
        if ( fabs(vel[i]) <= DRIVE_WHEEL_MIN_ERPM)
        {
            comm_can_set_duty(RudderChassis.DriveMotors.can_id[i],0);
        }
        else
        {
...
void SW_DriveMotors_SetSpeed(float vel[4])
{
    ...
    Delay_msec(1); // 发两个顿一下，不然CAN发送邮箱会炸
...
void SW_SteerMotors_GetAngle()
{
    for (int i = 0; i < 4; i++)
    {
//        dji_ReadInfo(1, i + 1);//此函数返回的量纲不对，而且不能保证每个电机信息都被收到
    }
}
