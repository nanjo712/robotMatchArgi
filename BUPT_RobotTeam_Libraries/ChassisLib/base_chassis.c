                              ...
                              float acc_ratio, float dec_ratio)
{
    ...
    float speed = 0;
    float max_speed = Max(Min(1.1 * total_distance,DRIVE_WHEEL_MAX_SPEED),DRIVE_WHEEL_MIN_SPEED); // 根据距离缩放速度 0.6 太快
...
//    if (BaseChassis.TrackStatus.point_index < point_num - 3) // 正常跑点逻辑
    if (BaseChassis.TrackStatus.point_index < point_num - 1) // 正常跑点逻辑
    {
